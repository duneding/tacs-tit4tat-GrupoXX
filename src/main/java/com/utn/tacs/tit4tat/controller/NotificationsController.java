...
public class NotificationsController {
	...
	List<Solicitud> getNotifications(
		...
		Usuario usuario = this.usuarioService.getUsuariosById(Long
				.parseLong(userId));
		List<Solicitud> notifications = this.solicitudService
				.getSolicitudesByUser(usuario);
	...
	public @ResponseBody
	void updateMessage(@RequestBody String request) {
		...
		Long idSolicitud = Long.parseLong(jsonRequest.get("id").toString());
		String message = jsonRequest.get("message").toString();
		
		this.solicitudService.updateMessage(idSolicitud, message);
	}
	...
	String create(@RequestBody String request) {
		...
		Long owner_id = Long.parseLong(jsonRequest.get("owner_id").toString());
		Long item_id = Long.parseLong(jsonRequest.get("item_id").toString());
		Long user_id = Long.parseLong(jsonRequest.get("user_id").toString());
		Long user_item_id = Long.parseLong(jsonRequest.get("user_item_id")
				.toString());
		String message = jsonRequest.get("message").toString();
		...
		// return "Solicitud de trueque " + sol.getId() +
	...
	public @ResponseBody
	int getNotificationsCount(@RequestParam(value = "userId") String userId) {
		int size = 0;
		...
		try {
			Usuario usuario = this.usuarioService.getUsuariosById(Long
					.parseLong(userId));
			List<Solicitud> notifications = this.solicitudService
					.getSolicitudesPendientesByUser(usuario);
	...
	public @ResponseBody
	String share(@PathVariable("idNotifications") String idNotifications) {
